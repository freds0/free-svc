...
class TextEncoder(nn.Module):
  ...
  def forward(self, x, x_lengths, f0=None, lang_id=None, noice_scale=1):
    ...
    # compute vuv vector
    if self.emb_uv:
      vuv_vector = torch.zeros(f0.size()).to(x.device)
      vuv_vector[f0 > 0.0] = 1.0
      vuv_vector[f0 <= 0.0] = 0.0
      uv_emb = self.emb_uv(vuv_vector.long().detach()).squeeze(1).transpose(1, 2)
    ...
    if self.lang_emb:
      x = x + self.lang_emb(lang_id).squeeze(1).transpose(1, 2)
