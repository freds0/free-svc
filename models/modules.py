...
class WN(torch.nn.Module):
    def __init__(self, 
        ...
        if pitch_channels != 0:
            cond_layer_pitch = torch.nn.Conv1d(
                pitch_channels, 2*hidden_channels*n_layers, 1)
            self.cond_layer_pitch = torch.nn.utils.weight_norm(
                cond_layer_pitch, name='weight')
    ...
    def forward(self, x, x_mask, g=None, pitch=None, **kwargs):
        ...
        if pitch is not None:
            pitch = self.cond_layer_pitch(pitch)
        ...
        for i in range(self.n_layers):
            ...
            if pitch is not None:
                cond_offset = i * 2 * self.hidden_channels
                g_l2 = pitch[:, cond_offset:cond_offset +
...
class ResidualCouplingLayer(nn.Module):
    def __init__(self,
        ...
        self.enc = WN(hidden_channels, kernel_size, dilation_rate, n_layers,
                      p_dropout=p_dropout, gin_channels=gin_channels, pitch_channels=pitch_channels)
        self.post = nn.Conv1d(
    ...
    def forward(self, x, x_mask, g=None, pitch=None, reverse=False):
        ...
        h = self.enc(h, x_mask, g=g, pitch=pitch)
