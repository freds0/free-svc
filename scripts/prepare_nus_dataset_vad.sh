...
function create_splits() {
    python3 scripts/preprocess_flist.py \
        --source-dir $DATASET_DIR_NAME/16k-segmented  \
...
# Function to segment audios
function segment() {
    python3 scripts/segment_vad.py \
        --dir $DATASET_DIR_NAME/16k \
        --out-dir ./$DATASET_DIR_NAME/16k-segmented
}

# Function to extract features
function extract_features() {
    python3 scripts/preprocess_spk.py \
        --in-dir $DATASET_DIR_NAME/16k-segmented \
...
set +x
echo "To easily train the model, rename the $DATASET_DIR_NAME folder to 'Data'"